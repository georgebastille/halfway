...
async function loadDatabase() {
  ...
  return new Promise((resolve, _reject) => {
    SQLite.openDatabaseAsync("myDatabaseName.db", "", "", "", (dbo) => {
      db = dbo;
  ...
  return new Promise((resolve, reject) => {
    ...
    db.transaction((tx) => {
      tx.executeSql(
        ...
        (_, error) => {
          console.log("Error getting Stations from DB");
          reject(error);
        },
      );
    });
...
async function getTop(values, comparator, callback) {
  ...
  for (var i = 0; i < 3; i++) {
    ...
    result += `${fullName}\n${option.mean.toFixed(1)} +/- ${
      2 * option.stdDev.toFixed(1)
    } minutes\n\n`;
  }
  callback(result);
}
  ...
  db.transaction(
    (tx) => {
      tx.executeSql(
        ...
        (errortx) => {
          console.log("Error with transaction:", errortx);
        },
      );
      //console.log('Done executing SQL');
    },
